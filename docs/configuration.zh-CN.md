...
原生新建线程能力按活跃度检测：正在运行、等待处理或刚刚活动的智能体每五分钟
刷新；最近七天用过的其他智能体每小时刷新；最近七天没有可见线程的智能体每天
刷新。Daemon 启动时会先检测一次。会话发现和手机重连不会越过这些间隔；真正的
`start_thread` 在启动前始终重新执行一次 fail-closed 检测。
