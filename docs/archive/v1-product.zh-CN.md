...
NekoNest 是**自托管的猫娘乐园**：在不向家用主机开入站端口、不取代各 agent 原生会话库的前提下，用**手机优先客户端**继续、驾驭并完成家中机器上**真实的本地编程 agent 线程**。**Codex 是 v1 唯一全控制 agent**；Claude Code、Kimi CLI、Grok Build 以**兼容续聊**适配器交付。Kilo 已退出现行支持；协议 1.x 只保留旧 wire id 的解析兼容。
...
| **多 agent 日常** | 一个乐园呈现 Codex（全控制）以及 Claude Code、Kimi、Grok（兼容续聊）；缺一个 CLI 不拖死全家 |
| **信任这个乐园** | 运维自有 VPS；**默认密封 E2E** 使中继读不到应用正文；开放中继仅管理员可开 |
...
1. **原生 store 为权威**（发现与 transcript，以及 agent 范围开线程后的所有权）。乐园侧 SQLite 是中继与耐久，不是永久分叉的第二套 agent 库。
2. **Daemon 主动出站**连接乐园。核心产品不要求家用主机入站端口。
...
8. **单个 agent 缺失对其余非致命。**  
9. **默认密封：** 新乐园使用 E2E 密封传输；开放中继须管理员显式配置；同一乐园的密封/开放客户端不得混用；禁止自动从密封降级为开放。
...
| **乐园服务器** | 鉴权、配对、多设备注册、手机身份/授权、WS 分发、密封耐久、附件（密封模式下密文）、推送 |
...
v1.0.0 = **单人运维的自托管乐园**可日常在路上使用的功能完备版本：含 **Codex 全控制**、**四个兼容续聊适配器**、**Windows 与 Linux**、**默认密封 E2E**。
...
| 领域 | v1 决策 |
|---|---|
| 传输 | E2E 密封为发版 **MUST**，且为新乐园**默认**。密封模式下 VPS 仅存密文。 |
| 开放中继 | 保留但**默认关闭**。仅服务器管理员可启用。每个乐园固定一种模式；密封/开放客户端不可混用；**禁止自动降级**。 |
...
| S13 | **E2E 密封通道** 手机↔daemon（中继仅见密文与已文档化的路由元数据） | MUST | **新乐园默认。** 开放中继仅管理员、显式、无自动回退 |
...
| P2 | 设置：乐园 URL + 管理员/引导 → 独立手机身份；不可达时清晰错误 | MUST | |
...
| L5 | 开线程生命周期状态 | MUST | `thread_starting` → `thread_owned` \| `thread_failed` \| `thread_indeterminate`。仅在首条提示词得到正向确认、且所选 agent 原生 store 正向确认所有权后设 `thread_owned`；否则为 `thread_indeterminate`。按设备 + 操作 id fail-closed 日志。indeterminate 后不自动重试。**无永久乐园侧幽灵行。** |
...
| L9 | 通用 `create_session` / 乐园虚构会话 | 禁止 | 不得重新引入 |

**不变量：** L2 成功（`thread_owned`）后，下一次发现必须能从所选 agent 的原生 store 找到该线程；手机不得永久持有主机无法 own 的线程。手机本地草稿不是乐园侧会话。不确定的开线程仅通过后续发现对账。
...
| X6 | 可信私有 VPS 的开放中继模式 | MUST | 仅管理员启用；乐园固定模式；README 警告；不混用/不回退 |
...
1. 运维部署乐园（TLS、管理员密钥、bootstrap）；传输模式密封（默认）。
...
1. 默认密封乐园；抽查乐园 DB/日志/附件存储无正文明文/文件名/审批细节。
2. 本地解密后聊天、审批与开线程仍可用。
...
| # | 主题 | 决议 |
|---|---|---|
| 1 | **E2E 默认** | 新乐园默认密封。开放中继保留、默认关闭、**仅管理员**启用。每个乐园只有一种模式；不混用；不自动降级。 |
...
*当单人运维能部署默认密封的乐园、在 Windows 与 Linux 上连接家用机、为四个现行 agent 续接原生线程、对 Codex 完成全控制闭环（通知 → 审批/steer/中断/在已发现目录开线程 → 完成）、对其余三个诚实兼容续聊、在已文档化的元数据边界内信任投递与密封传输——且产品没有滑成远程 IDE、云端 agent、或对每个 CLI 同等全控制承诺——NekoNest v1.0.0 即告完成。*
