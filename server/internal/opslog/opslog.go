// Package opslog provides the server's privacy-safe operational logging.
package opslog
...
// Config controls the process-wide structured log handler.
type Config struct {
	Format string
	Level  slog.Level
}

// ParseConfig validates the two supported logging environment values.
...
// Configure initializes slog from NEKONEST_LOG_FORMAT and NEKONEST_LOG_LEVEL.
...
// New builds a logger suitable for tests or process initialization.
...
// Error records only a stable error class. Error strings are intentionally not
// emitted because upstream error values can contain credentials or user data.
...
// RedirectStandard prevents unexpected dependency log output from leaking
// untrusted content while this package owns the process logger.
