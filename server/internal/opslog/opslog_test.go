...
func TestFormatsLevelsAndInvalidConfig(t *testing.T) {
	for _, format := range []string{"text", "json"} {
		...
		Info("server.test", "format_checked", "format checked")
...
func TestJSONEmitsAllFourLevels(t *testing.T) {
	...
	Debug("server.test", "debug_event", "debug message")
	Info("server.test", "info_event", "info message")
	Warn("server.test", "warn_event", "warn message")
	Error("server.test", "error_event", "error message", errors.New("secret error body"))
...
func TestConcurrentEventsHaveStableFieldsAndRedactErrors(t *testing.T) {
	...
	for range workers {
		...
		go func() {
			defer wg.Done()
			Error("server.test", "write_failed", "write failed", errors.New("token-sentinel prompt-sentinel /secret/path"))
		}()
	}
	wg.Wait()
	lines := strings.Split(strings.TrimSpace(out.String()), "\n")
	...
	for _, line := range lines {
		...
		if record["component"] != "server.test" || record["event"] != "write_failed" || record["error"] != "operation_failed" {
...
func TestWireIdentifiersAreValidatedBeforeLogging(t *testing.T) {
	...
	Info("server.test", "identifier_checked", "identifier checked",
